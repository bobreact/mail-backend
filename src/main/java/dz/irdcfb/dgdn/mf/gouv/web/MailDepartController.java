...
import dz.irdcfb.dgdn.mf.gouv.dao.MailDepartRepository;
...
public class MailDepartController {
	...
	public List<MailDepart> getAllMails() {
		return mailDepartService.getAllMail();
	}
	...
	public ResponseEntity<?> saveMail(@RequestPart("mail") @Valid MailDepart mail,
		...
		if (mailDepartRepository.existsByNumDepartAndAnnee(mail.getNumDepart(),
				dateDepart.toInstant().atZone(ZoneId.systemDefault()).toLocalDate().getYear())) {
